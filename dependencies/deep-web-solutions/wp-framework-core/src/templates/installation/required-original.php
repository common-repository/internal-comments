...
">
	...
	echo wp_kses_post(
		...
		/* translators: 1. Plugin name, 2. Plugin version, 3. Name of the install button */
			__( '<strong>%1$s (v%2$s)</strong> needs to run its installation routine before it can be used. Please click the "%3$s" button to proceed:', 'internal-comments' ),
			$this->get_plugin()->get_plugin_name(),
			$this->get_plugin()->get_plugin_version(),
			/* translators: Name of the installation button */
			__( 'Install', 'internal-comments' )
		)
	);
	?>
</p>
<p>
	...
	">
		<?php
		esc_html_e( 'Install', 'internal-comments' );
		?>
	</button>
</p>
<?php
