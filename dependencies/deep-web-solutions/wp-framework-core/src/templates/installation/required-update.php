...
">
	...
	echo wp_kses_post(
		...
		/* translators: 1. Plugin name, 2. Plugin version, 3. Name of the update button */
			__( 'A data update is available for <strong>%1$s (v%2$s)</strong>. It is recommended to back up your database before proceeding. Please click the "%3$s" button when ready:', 'internal-comments' ),
			$this->get_plugin()->get_plugin_name(),
			$this->get_plugin()->get_plugin_version(),
			/* translators: Name of the update button */
			__( 'Update', 'internal-comments' )
		)
	);
	?>
</p>
<p>
	<button
		...
		">
		<?php
		esc_html_e( 'Update', 'internal-comments' );
		?>
	</button>
</p>
<?php
