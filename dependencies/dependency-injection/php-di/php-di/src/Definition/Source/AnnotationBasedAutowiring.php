...
namespace DWS_IC_Deps\DI\Definition\Source;

use DWS_IC_Deps\DI\Annotation\Inject;
use DWS_IC_Deps\DI\Annotation\Injectable;
use DWS_IC_Deps\DI\Definition\Exception\InvalidAnnotation;
use DWS_IC_Deps\DI\Definition\ObjectDefinition;
use DWS_IC_Deps\DI\Definition\ObjectDefinition\MethodInjection;
use DWS_IC_Deps\DI\Definition\ObjectDefinition\PropertyInjection;
use DWS_IC_Deps\DI\Definition\Reference;
use DWS_IC_Deps\Doctrine\Common\Annotations\AnnotationRegistry;
use DWS_IC_Deps\Doctrine\Common\Annotations\Reader;
use DWS_IC_Deps\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use DWS_IC_Deps\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'DWS_IC_Deps\\DI\\Annotation\\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'DWS_IC_Deps\\DI\\Annotation\\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('DWS_IC_Deps\\DI\\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'DWS_IC_Deps\\DI\\Annotation\\Injectable');
